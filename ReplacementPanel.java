...
public class ReplacementPanel extends JPanel
{
		...
		for (intRow = 0; intRow < 12; intRow++)
		{
			...
			for (intColumn = 0; intColumn < 11 && intDrawX <= 600; intColumn++)
			{
				if (strSettlements[intRow][intColumn].equals("r") || strSettlements[intRow][intColumn].equals("b")
						|| strSettlements[intRow][intColumn].equals("w")
						|| strSettlements[intRow][intColumn].equals("o"))
				{
	...
	public ReplacementPanel()
	{
		...
		try
		{
			...
			f48 = Font.createFont(Font.TRUETYPE_FONT, new FileInputStream(new File("ipixelu.ttf")))
					.deriveFont(Font.PLAIN, 48);
			f24 = Font.createFont(Font.TRUETYPE_FONT, new FileInputStream(new File("ipixelu.ttf")))
