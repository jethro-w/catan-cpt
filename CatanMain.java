 ...
 * <h1>This game is Settlers of Catan by Justin, Jethro, and Jason written for
 * the ICS4U1 course. Users play online in a strategy game against friends
 * 
...
public class CatanMain implements ActionListener, MouseMotionListener, KeyListener, MouseListener
{
	...
	/*
	 * public static String[][] loadMap () throws IOException { BufferedReader map =
	 * null; String[] strMapLine = new String[5]; String[][] strMap = new
	 * String[5][9]; int intCount;
	 * 
	 * try { map = new BufferedReader(new FileReader("catan-tiles.csv")); } catch
	 * (IOException e) {
	 * 
	 * }
	 * 
	 * for (intCount = 0; intCount < 5; intCount++) { try { strMapLine[intCount] =
	 * map.readLine(); } catch (IOException e) { } strMap[intCount] =
	 * strMapLine[intCount].split(","); }
	 * 
	 * return strMap; }
	 * 
	 * public static String[][] loadSettlements () throws IOException {
	 * BufferedReader map = null; String[] strMapLine = new String[12]; String[][]
	 * strMap = new String[12][11]; int intCount;
	 * 
	 * try { map = new BufferedReader(new FileReader("catan-settlements-map.csv"));
	 * } catch (IOException e) {
	 * 
	 * }
	 * 
	 * for (intCount = 0; intCount < 12; intCount++) { try { strMapLine[intCount] =
	 * map.readLine(); } catch (IOException e) { } strMap[intCount] =
	 * strMapLine[intCount].split(","); }
	 * 
	 * return strMap; }
	 * 
	 * public static String[][] loadRoads () throws IOException { BufferedReader map
	 * = null; String[] strMapLine = new String[11]; String[][] strMap = new
	 * String[11][11]; int intCount;
	 * 
	 * try { map = new BufferedReader(new FileReader("catan-roads-map.csv")); }
	 * catch (IOException e) {
	 * 
	 * }
	 * 
	 * for (intCount = 0; intCount < 11; intCount++) { try { strMapLine[intCount] =
	 * map.readLine(); } catch (IOException e) { } strMap[intCount] =
	 * strMapLine[intCount].split(","); }
	 * 
	 * return strMap; }
	...
	public void actionPerformed (ActionEvent evt)
	{
		...
		else if (evt.getSource() == buttonPort)
		{
			try
			{
				...
				if (strPort.length() == 4)
				{
					...
					panel.blnMainMenu = true;
					panel.blnSettings = false;
		...
		*/
		...
		{
			panel.intHelp++;
			System.out.println(panel.intHelp);
			if (panel.intHelp > 4)
			{
				panel.blnMainMenu = true;
	...
	public void mouseClicked (MouseEvent evt)
	{
		...
		/*
		 * int intXCell; // Column int intYCell; // Row int intRow;
		 * 
		 * intMouseX = evt.getX(); intMouseY = evt.getY();
		 * 
		 * System.out.println(intMouseX + ", " + intMouseY);
		 * 
		 * if (drawSettlement == true) { intDrawY = 90; for (intRow = 1; intRow <= 12;
		 * intRow++) { if (intRow == 1 || intRow == 12) { intDrawX = 240; } else if
		 * (intRow == 2 || intRow == 3 || intRow == 10 || intRow == 11) { intDrawX =
		 * 190; } else if (intRow == 4 || intRow == 5 || intRow == 8 || intRow == 9) {
		 * intDrawX = 140; } else if (intRow == 6 || intRow == 7) { intDrawX = 90; }
		 * 
		 * for (intCount = 0; intDrawX <= 590; intDrawX = intDrawX + 100) { if
		 * (intMouseX >= intDrawX && intMouseX <= intDrawX + 20 && intMouseY >= intDrawY
		 * && intMouseY <= intDrawY + 20) { intXCell = (int) Math.round((intMouseX -
		 * 100) / 50.0); intYCell = (int) Math.round((intMouseY / 43.0) - 2.1);
		 * 
		 ...
		 * 
		 * // ADD FEATURE TO DISABLE SURROUNDING SPOTS (dependent on row number) if
		 * (intRow == 1 || intRow == 3 || intRow == 5 || intRow == 7 || intRow == 9 ||
		 * intRow == 11) { thepanel.strSettlements[intYCell + 1][intXCell + 1] = "x";
		 * thepanel.strSettlements[intYCell + 1][intXCell - 1] = "x";
		 * strSettlements[intYCell + 1][intXCell + 1] = "x"; strSettlements[intYCell +
		 * 1][intXCell - 1] = "x"; if (intRow != 1) { thepanel.strSettlements[intYCell -
		 * 1][intXCell] = "x"; strSettlements[intYCell - 1][intXCell] = "x"; } } else if
		 * (intRow == 2 || intRow == 4 || intRow == 6 || intRow == 8 || intRow == 10 ||
		 * intRow == 12) { thepanel.strSettlements[intYCell - 1][intXCell + 1] = "x";
		 * thepanel.strSettlements[intYCell - 1][intXCell - 1] = "x";
		 * strSettlements[intYCell - 1][intXCell + 1] = "x"; strSettlements[intYCell -
		 * 1][intXCell - 1] = "x"; if (intRow != 12) { thepanel.strSettlements[intYCell
		 * + 1][intXCell] = "x"; strSettlements[intYCell + 1][intXCell] = "x"; } }
		 * 
		 * // ADD IF STATEMENTS TO ONLY ALLOW SETTLEMENTS CONNECTED TO ROAD SEGMENTS } }
		 * } if (intDeltaY == 30) { intDeltaY = 56; } else if (intDeltaY == 56) {
		 * intDeltaY = 30; }
		 * 
		 * intDrawY = intDrawY + intDeltaY; } } else if (drawRoad == true) { intDeltaY =
		 * 56; intDrawY = 100; for (intRow = 1; intRow <= 11 ; intRow++) { if (intRow ==
		 * 1 || intRow == 11) { intDrawX = 200; intDeltaX = 50; } else if (intRow == 2
		 * || intRow == 10) { intDrawX = 185; intDeltaX = 100; } else if (intRow == 3 ||
		 * intRow == 9) { intDrawX = 150; intDeltaX = 50; } else if (intRow == 4 ||
		 * intRow == 8) { intDrawX = 135; intDeltaX = 100; } else if (intRow == 5 ||
		 * intRow == 7) { intDrawX = 100; intDeltaX = 50; } else if (intRow == 6) {
		 * intDrawX = 85; intDeltaX = 100; }
		 * 
		 * for (intCount = 0; intDrawX <= 590; intDrawX = intDrawX + intDeltaX) { if
		 * (intRow == 1 || intRow == 3 || intRow == 5 || intRow == 7 || intRow == 9 ||
		 * intRow == 11) { if (intMouseX >= intDrawX && intMouseX <= intDrawX + 50 &&
		 * intMouseY >= intDrawY && intMouseY <= intDrawY + 30) { intXCell = (int)
		 * Math.floor((intMouseX - 100) / 50.0); intYCell = (int) Math.round((intMouseY
		 * / 43.0) - 2.8);
		 * 
		 ...
		 * System.out.println(intMouseX + ", " + intMouseY); } } } else if (intRow == 2
		 * || intRow == 4 || intRow == 6 || intRow == 8 ||intRow == 10) { if (intMouseX
		 * >= intDrawX && intMouseX <= intDrawX + 30 && intMouseY >= intDrawY &&
		 * intMouseY <= intDrawY + 56) { intXCell = (int) Math.round((intMouseX - 100) /
		 * 50.0); intYCell = (int) Math.round((intMouseY / 43.0) - 2.8);
		 * 
	...
	public CatanMain()
	{
		...
		buttonClient.setFont(panel.f24);
		...
		labelServerIP.setFont(panel.f24);
